...
// SDK 的分布式限流走 RedisLimiter（ZSET 滑动窗口），此类型不被 SDK 内部调用，
// 作为工具类导出供业务侧在不依赖 Redis 的场景下独立使用。
...
// NewSlidingWindowLimiter 创建滑动窗口限流器。window 通常为 1s，maxRequests 为窗口内允许次数（QPS）。
